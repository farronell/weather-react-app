...
import useDataFetch from "../State/useDataFetch";
import Form from "./Form";
import WeatherInfo from "./WeatherInfo";

export default function WeatherCard() {
  const [city, setCity] = useState("Kyiv");
  const { data, error } = useDataFetch(city);
  ...
  if (error) {
    return (
      <div>
        <h2>Error fetching weather data: {error.message}</h2>
      </div>
    );
  }

  return (
    <section className="items-center flex flex-col justify-center h-dvh">
